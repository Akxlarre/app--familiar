...
@Component({
  ...
  template: `
    <div #container class="flex flex-col gap-6 font-body">
      <app-page-header
        title="Mi cuenta"
        description="Presupuesto personal por categoría. Solo tú ves esta información."
      >
      ...
      @if (loading()) {
        ...
        @if (personalAccountIds().length === 0) {
          <app-empty-state
            message="Sin cuentas personales"
            subtitle="En Finanzas → Cuentas, edita una cuenta y asígnate como dueño para que aparezca aquí."
            icon="pi-user"
          />
        } @else {
          <!-- Resumen total -->
          ...
          <section class="rounded-xl border border-default bg-surface p-4 shadow-sm space-y-4">
            ...
            <p class="text-sm text-secondary">Administra en qué categorías gastas tu dinero personal.</p>
...
export class MiCuentaComponent implements OnInit, AfterViewInit {
  ...
  personalAccountIds = signal<string[]>([]);
  ...
  private async loadData(): Promise<void> {
    ...
    this.personalAccountIds.set(accountIds);
    ...
    const [summaryRes, budgetRes, spentRes, catRes] = await Promise.all([
      ...
      this.budgetService.getBudgets(householdId, this.selectedYear, this.selectedMonth, profileId),
      this.transactionService.getExpensesByCategoryForMonth(householdId, this.selectedYear, this.selectedMonth, accountIds),
